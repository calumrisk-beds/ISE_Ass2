...
class ClosedBookings:
    def __init__(self, master, uid_token, slcted_tl):
        ...
        if self.usr_type == "Tool Owner":
            self.my_closed_bkgs = sqlr.get_closed_bookings_by_tid(self.slcted_tl)
        elif self.usr_type == "Dispatch Rider":
            self.my_closed_bkgs = sqlr.get_closed_bookings_by_courier(self.uid_token)
        else:
            self.my_closed_bkgs = sqlr.get_closed_bookings_by_uid(self.uid_token)
